...
class App extends Component {
  state = { loaded: false, kycAddress: "0xCD40" , tokenSaleAddress:null, userTokens: 0 };
  ...
  handleKycSubmit = async () => {
    ...
    console.log("kycAddress", kycAddress, this.accounts[0], this.kycContract)
    await this.kycContract.methods.setKycCompleted(kycAddress).send({from: this.accounts[0]});
    alert("Account "+kycAddress+" is now whitelisted");
  }

  handleBuyToken = async () => {
    await this.myTokenSale.methods.buyTokens(this.accounts[0]).send({from: this.accounts[0], value: 1000000000});
  }
